...
class TweetSearch extends AbstractController
{
    ...
    public function addEndTime(string $date): TweetSearch
    {
        $this->endTime = $date;
        return $this;
    }
	
    public function addStartTime(string $date): TweetSearch
    {
        $this->startTime = $date;
        return $this;
    }
    ...
    protected function constructEndpoint(): string
    {
        ...
        if (!empty($this->endTime)) {
            $endpoint .= '&end_time=' . date('Y-m-d\TH:i:s.000\Z', strtotime($this->endTime));

        }

        if (!empty($this->startTime)) {
            $endpoint .= '&start_time=' . date('Y-m-d\TH:i:s.000\Z', strtotime($this->startTime));
