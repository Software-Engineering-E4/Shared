...
class Requester(ABC):
    ...
    def send_to_db(self, data: dict[str, str | int], format) -> None:
        formatted = self.db.format_data(data, format)
        ...
        self.logger.debug(f"{self.send_mode.__str__()} on row with id={data['id']}")
